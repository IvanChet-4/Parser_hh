...
public class Parse_Controller {
    ...
    @GetMapping("all")
    public ResponseEntity getParsers() { // Вывод данных из БД на экран при нажатии "Вывести все значения из БД на экран"
    ...
    @PostMapping("add")
    public String startParse(@ModelAttribute ArrayList<Resume_Entity> resume_entity, @ModelAttribute Resume_Entity resume_entity_set) { // Парсинг, запись в БД при нажатии "Запустить парсинг, добавить значения в БД"
        try {
            for (int j = 0; j < Jtoe; j++) { // j - номер страницы, на каждой странице отображено 100 резюме
                URL url = new URL(SITE_URL1 + j + SITE_URL2); // открываем соединение с сайтом (в константе SITE_URL зашит поисковый запрос, который можно изменить)
                Document page = Jsoup.parse(url, 7000); // находим ссылку на резюме и достаем необходимую ее часть
...
*/
                    ...
                    resume_entity.add(resume_entity_set); // Записываем результат
                }
            } // Закрываются циклы for
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
        startSave(resume_entity); // Сохраняем результат
        return "БД обновлена";
    }

    private void startSave(ArrayList<Resume_Entity> resume_entity) { // Сохранение результатов парсинга в БД
